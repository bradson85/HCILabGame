		...
		//=======================================================================================	
		 var name =$_GET['gamename'];
	        ...
	        if (list <= 3){
	    	 ...
	    	 for (i=1; i <= value; i++){
                     $('#boxentrycontainer').append('<label for="textboxes">Category ' + i +' Name: &nbsp;</label>'
	     				 + '<input type = "text" id = textbox'+i+' class= "textboxes"'
	     				 + 'placeholder= "Enter Category ' + i +' Name" maxlength="25" name = "category'+ i +'"> <br>' );
	    	 }
			...
			var handle = $( "#custom-handle" );
					    ...
					    change: function(event, ui) {
					    	...
					    	$('#boxentrycontainer').empty();
					    	 for (i=1; i < value +1 ; i++){
					    		 ...
					    		 else {	$('#boxentrycontainer').append('<label for="textboxes">Category ' + i +' Name:&nbsp;&nbsp;</label>'
					    		 				 + '<input type = "text"  id= "textbox' +i+'"class= "textboxes"'
					    		 				 + 'placeholder= "Enter Category ' + i +' Name" maxlength="25" name = "category'+ i +'"> <br>' );
					    			 }}
					    	 for(j = 1; j <= list ; j++){
			 ...
			 var creatorstatus = ['Filename','Scoring','Contestants','Categories','Questions'];
				...
				var table = $('#statusbarcategories').children();
					for(j = 0; j < 5; j++){
						if(j==3){table.append('<td class = "progressbarmarked">' + (j+1) + '</td><td><img id= "dotarrow" src = "../images/dotarrow.png"></td>');
				...
				$('#statusbarcategories').append('<tr></tr>');
					for(j = 0; j < 5; j++){
						if(j==3){table.append('<td class = "progressbarmarkedbelow">' + creatorstatus[j] + '</td><td>&nbsp;</td>');$(".progressbarmarkedbelow").css("color" , "Black");}
